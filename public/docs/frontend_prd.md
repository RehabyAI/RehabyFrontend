...
REHABY
Frontend PRD & API Documentation
Version 1.0    May 14–19, 2026    Hackathon Build

PART 1
Frontend PRD
This document defines what the frontend team builds, who builds what, and when it must be done. No scope beyond this document.
...
Tech Stack
React + Vite  |  TailwindCSS  |  MediaPipe.js (client-side pose)  |  Recharts  |  Axios

2. Screens & Components
Screen 1 — Patient Intake  (samkiel's)
Route: /  |  Owner: samkiel
...
Screen 2 — Exercise Session  (samkiel's — CORE SCREEN)
Route: /session  |  Owner: samkiel
...
Screen 3 — Session Summary  (samkiel's)
Route: /summary  |  Owner: samkiel
Show: avg form score, total reps, duration, top 3 errors detected
Simple card layout, no chart needed
Button: 'Back to Home'

Screen 4 — Clinician Dashboard  (John)
Route: /clinician  |  Owner: John
...
Screen 5 — Navigation / Layout  (John)
Route: shared layout  |  Owner: John
...
HIGH
Day 3–4
Form score + rep counter UI
samkiel
HIGH
Day 4 (May 17)
Audio feedback (Web Speech)
samkiel
MED
Day 4
...
Base URL (local dev): http://localhost:8000  |  All responses: JSON
...
POST
/analyze-frame
Send pose landmarks, get score + feedback
POST
/end-session
...
GET
/patient/{id}/analytics
Score trends + error frequency
GET
/session/{id}/details
Full detail of one session

If backend is not ready
Mock all endpoints with static JSON in /src/mocks/api.js so frontend dev is never blocked.

6. Team Rules
samkiel scaffold the repo and push to GitHub. John clones.
Never work on the same file at the same time — divide by route.
Daily standup message in group chat: what samkiel did, what's next, any blockers.
If blocked for more than 2 hours, flag it immediately — don't wait.
No new features after May 16. Only polish and bug fixes after that.